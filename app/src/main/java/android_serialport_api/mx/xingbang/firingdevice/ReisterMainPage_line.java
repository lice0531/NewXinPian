...
import android_serialport_api.mx.xingbang.cmd.vo.From12Reister;
...
import android_serialport_api.mx.xingbang.db.Denator_type;
...
//        Utils.saveFile();//把软存中的数据存入磁盘中
     ...
     * 获取第一发雷管
     */
    private DenatorBaseinfo serchLastLG() {
        List<DenatorBaseinfo> list = new GreenDaoMaster().queryDetonatorRegionDesc(mRegion);
        if (list.size() > 0) {
