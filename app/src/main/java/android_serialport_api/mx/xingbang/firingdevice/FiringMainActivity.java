...
import android_serialport_api.mx.xingbang.cmd.ThreeFiringCmd;
import android_serialport_api.mx.xingbang.cmd.vo.From32DenatorFiring;
import android_serialport_api.mx.xingbang.cmd.vo.From42Power;
import android_serialport_api.mx.xingbang.cmd.vo.To52Test;
import android_serialport_api.mx.xingbang.custom.ErrListAdapter;
...
import android_serialport_api.mx.xingbang.db.DenatorHis_Main;
import android_serialport_api.mx.xingbang.db.GreenDaoMaster;
import android_serialport_api.mx.xingbang.db.MessageBean;
import android_serialport_api.mx.xingbang.db.greenDao.DenatorBaseinfoDao;
import android_serialport_api.mx.xingbang.db.greenDao.MessageBeanDao;
import android_serialport_api.mx.xingbang.jilian.FirstEvent;
import android_serialport_api.mx.xingbang.models.VoDenatorBaseInfo;
import android_serialport_api.mx.xingbang.models.VoFireHisMain;
import android_serialport_api.mx.xingbang.models.VoFiringTestError;
import android_serialport_api.mx.xingbang.utils.CommonDialog;
...
public class FiringMainActivity extends SerialPortActivity {
    ...
    private TextView tv_set_delay;
    private NiceSpinner nsDuanWei;
    ...
    private void initView() {
        ...
        String miaoliang = (String) SPUtils.get(this,"miaoliang","0");
        ...
        tv_set_delay=findViewById(R.id.tv_set_delay);
        ...
        btn_lowVoltage.setOnClickListener(v -> {
            ...
            if(Utils.isNumber(str_lowVoltage)){
                MmkvUtils.savecode("lowVoltage", str_lowVoltage);
        ...
        btn_highVoltage.setOnClickListener(v -> {
            String str_highVoltage=  et_set_highVoltage.getText().toString();
            int high = Double.valueOf(Utils.convertToDouble(str_highVoltage,16.0)*10).intValue();
            Log.e("设置电压", "low: "+high );
            if(Utils.isNumber(str_highVoltage)){
                MmkvUtils.savecode("highVoltage", str_highVoltage);
        ...
        btn_start.setOnClickListener(v -> {
            SPUtils.put(FiringMainActivity.this,"miaoliang",tv_set_delay.getText().toString());
//            MmkvUtils.savecode("miaoliang",tv_set_delay.getText().toString());
            ll_0.setVisibility(View.GONE);
            if (!firstThread.isAlive()){//从onstart挪过来的
                firstThread.start();
            }
        });
        nsDuanWei = findViewById(R.id.ns_duanwei);
        ...
        dList.add("4段");
        dList.add("5段");
        nsDuanWei.attachDataSource(dList);
        Log.e(TAG,"miaoliang:" + miaoliang);
        if (!TextUtils.isEmpty(miaoliang)) {
            switch (miaoliang) {
                case "0":
                    nsDuanWei.setSelectedIndex(0);
                    break;
                case "25":
                    nsDuanWei.setSelectedIndex(1);
                    break;
                case "50":
                    nsDuanWei.setSelectedIndex(2);
                    break;
                case "75":
                    nsDuanWei.setSelectedIndex(3);
                    break;
                case "100":
                    nsDuanWei.setSelectedIndex(4);
                    break;
            }
            tv_set_delay.setText(miaoliang);
        } else {
            tv_set_delay.setText("0");
        }
        nsDuanWei.setOnSpinnerItemSelectedListener(new OnSpinnerItemSelectedListener() {
            @Override
            public void onItemSelected(NiceSpinner parent, View view, int position, long id) {
                Log.e(TAG,dList.get(position));
                switch (position) {
                    case 0:
                        //1段
                        tv_set_delay.setText("0");
                    ...
                    case 1:
                        //2段
                        tv_set_delay.setText("25");
                        break;
                    case 2:
                        //3段
                        tv_set_delay.setText("50");
                        break;
                    case 3:
                        //4段
                        tv_set_delay.setText("75");
                        break;
                    case 4:
                        //5段
                        tv_set_delay.setText("100");
                        break;
                }
            }
        });
...
//                        .setNeutralButton("确定", (dialog12, which) -> dialog12.dismiss())
                ...
                eightTxt.setText(msg.obj.toString());
            }
            return false;
        });
        //接受消息之后更新imageview视图
...
//        List<DenatorBaseinfo> denatorlist = master.queryDenatorBaseinfo();
        ...
        Log.e(TAG,"LgNum: " + denatorlist.size());
...
//                }
//            }
            byte[] initBuf = ThreeFiringCmd.setCmd_3A("00");//3A秒量命令
            sendCmd(initBuf);
...
//                }
            ...
            Log.e("起爆页面", "发送46指令");
        } else if (DefCommand.CMD_5_TEST_8.equals(cmd)) {//5B设置低压
            Message msg = Handler_tip.obtainMessage();
            msg.what = 4;
            Handler_tip.sendMessage(msg);
...
//                                short delayTime = write.getDelay();
                                short delayTime = Short.parseShort(tv_set_delay.getText().toString());
...
//                                    sendCmd(FourStatusCmd.send46("00", "01", denatorCount));//20(快)
                                ...
                                writeDenator = write;
                                thirdWriteCount++;
                                Log.e(TAG, "case33--38指令已发完--thirdWriteCount: " + thirdWriteCount);
                                mHandler_1.sendMessage(mHandler_1.obtainMessage());
                            } else {
                                long thirdEnd = System.currentTimeMillis();
                                long spanTime = thirdEnd - thirdStartTime;
